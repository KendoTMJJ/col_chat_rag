...
class SupabaseChatMemory(BaseChatMessageHistory):
    def __init__(self, session_id: str):
        self.session_id = session_id

    @property
    def messages(self) -> List[BaseMessage]:
        """Trae los mensajes del Supabase"""
        response = supabase.table("historial_chat").select(
            "historial"
    ...
    def clear(self) -> None:
        ...
        supabase.table("historial_chat").delete().eq(
            "session_id", self.session_id).execute()
